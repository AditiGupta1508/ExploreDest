// ============================================================
// ExploreDest - Enhanced App.js v3
// Author: Aditi Gupta
// Changes: Added recommendedDays, smart multi-destination 
//          combining, day-by-day itinerary generator,
//          domestic/international toggle
// ============================================================

// 1. EXPANDED DESTINATION DATABASE with recommendedDays
const destinations = [
    // --- PUNJAB / HIMACHAL ---
    {
...
async function planMyTrip() {
    ...
    try {
        if (!originCoords) {
            const geoRes = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(startCity)}&format=json&limit=1`);
            const geoData = await geoRes.json();
            if (geoData.length === 0) throw new Error("City not found");
            originCoords = { lat: parseFloat(geoData[0].lat), lng: parseFloat(geoData[0].lon) };
        }

        // Step 1: Calculate cost and filter affordable destinations
        ...
        // Step 2: Find smart trip combinations that match the days
        const tripOptions = findTripOptions(affordableDestinations, days, filter);

        // Step 3: Fetch Wikipedia images for all destinations
        const allDests = [...new Set(affordableDestinations)];
        ...
        await Promise.all(allDests.map(async (dest) => {
            try {
        ...
        container.innerHTML = `<div class="empty-state"><h3>⚠️ City not found. Please check the spelling.</h3></div>`;
        originCoords = null;
    }
}
...
function renderResults(tripOptions, days) {
    ...
    if (tripOptions.length === 0) {
        container.innerHTML = `<div class="empty-state">
            <h3>💸 Budget a bit tight?</h3>
            <p>Try reducing your days by 1, increasing budget slightly, or switching between domestic and international options!</p>
        </div>`;
        return;
    }
    ...
    tripOptions.forEach((option, index) => {
        ...
        const description = isSingle
            ? dests[0].description
            : `A perfect ${totalDays}-day combo — explore two incredible destinations on one trip!`;
...
async function openGuide(destKey, totalDays) {
    ...
    const title = isSingle ? `Your ${totalDays}-Day Trip to ${dests[0].name}` : `Your ${totalDays}-Day ${dests[0].name} + ${dests[1].name} Combo`;

    document.getElementById('guideTitle').innerText = title;

    // Transport & Budget info
    const totalCostUSD = dests.reduce((sum, d) => sum + d.totalCostUSD, 0);
    ...
    document.getElementById('guideBudget').innerHTML = `
        ...
        <b>Total Trip Cost:</b> ${formatMoney(totalCostUSD)}<br>
    ...
    for (const dest of dests) {
        ...
        // Day plans from the destination data
        for (const plan of dest.dayPlan) {
            gemsList.innerHTML += `
                <li>
                    📅 <b>Day ${dayNumber}</b> — ${plan}
                </li>`;
            dayNumber++;
        }
        ...
        for (const gem of dest.gems) {
            ...
            try {
                ...
                let imgSrc = wikiData.originalimage?.source || wikiData.thumbnail?.source?.replace(/\/\d+px-/, '/600px-') || "";
    ...
    try {
        ...
        grid.innerHTML = "";
        for (let i = 0; i < 7; i++) {
            const dateStr = new Date(data.daily.time[i]).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
            grid.innerHTML += `
